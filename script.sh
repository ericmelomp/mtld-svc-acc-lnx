...
CMDS=(
    ...
    "sudo find /etc/ssh/sshd_config.d/ -type f -name '*.conf' -exec sed -i 's/PasswordAuthentication no/PasswordAuthentication yes/g' {} + 2>/dev/null || true"
    "sudo sed -i 's/^#* *PasswordAuthentication.*/PasswordAuthentication yes/' /etc/ssh/sshd_config 2>/dev/null; sudo sed -i 's/^PasswordAuthentication no/PasswordAuthentication yes/g' /etc/ssh/sshd_config 2>/dev/null; true"
    "sudo sshd -t 2>/dev/null && (sudo systemctl restart ssh 2>/dev/null || sudo systemctl restart sshd 2>/dev/null) || true"
